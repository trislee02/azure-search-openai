...
from approaches.promptutils import ChatRAGPrompt, ChatTeacherStudentPrompt

class ChatRAGTeacherStudentApproach(Approach):
    """
    This approach includes following steps:
        ...
        2. Teacher-Student self-refining
            a. Generate feedback based on question, first answer, and retrieved documents
            b. Refine answer based on feedback, question, first answer, and retrieved documents
    """
    ...
    def run(self, history: Sequence[dict[str, str]], overrides: dict[str, Any]) -> Any:
        ...
        user_conv = ChatRAGPrompt.user_chat_template.format(content=content, question=history[-1]["user"])
        ...
        response_to_check = ChatTeacherStudentPrompt.response_check_template.format(source=content,
                                                                      question=history[-1]["user"],
                                                                      answer=chat_content)
                
        messages = [{"role":"system","content": ChatTeacherStudentPrompt.system_message_check_response},
                  {"role":"user","content": response_to_check}]

        chat_completion = self.__compute_chat_completion(messages=[{"role":"system","content": ChatTeacherStudentPrompt.system_message_check_response},
                                                                   {"role":"user","content": response_to_check}], 
                                                         temperature=overrides.get("temperature") or 0.0, 
                                                         max_tokens=1024, 
                                                         n=1)
        ...
        if feedback.strip() != '0' and feedback.strip() != '1': # If LLM doesn't know the answer or answers correctly, skip the revision
            response_to_revise = ChatTeacherStudentPrompt.response_revise_template.format(source=content,
                                                                            question=history[-1]["user"],
                                                                            previous_answer=chat_content,
                                                                            feedback=feedback)
            messages = [{"role":"system","content": ChatTeacherStudentPrompt.system_message_revise_prompt},
                    {"role":"user","content": response_to_revise}]

            chat_completion = self.__compute_chat_completion(messages=[{"role":"system","content": ChatTeacherStudentPrompt.system_message_revise_prompt},
                                                                    {"role":"user","content": response_to_revise}], 
                                                            temperature=overrides.get("temperature") or 0.0, 
                                                            max_tokens=1024, 
                                                            n=1)
