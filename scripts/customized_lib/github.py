...
class GitHubIssuesLoader(BaseGitHubLoader):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        while url:
            ...
            for issue in issues:
                ...
                comments_api_url = doc.metadata["comments_url"]
                comments_resp = requests.get(comments_api_url, headers=self.headers)
                comments_resp.raise_for_status()
                comments_response = comments_resp.json()
                comments = self.parse_comments(comments_response)
                doc.metadata["list_comments"] = comments
                if not self.include_prs and doc.metadata["is_pull_request"]:
                    continue
                doc = self.merge_comments_to_doc(doc)
                yield doc
    ...
    def parse_comments(self, comments: dict) -> list:
        """Create a list of comment strings of an issue"""
        comments_list = [c["body"] for c in comments]
        return comments_list
    
    def merge_comments_to_doc(self, doc: Document) -> Document:
        """Parse a document by merging lists of comments into page_content"""
        issue = doc.page_content
        comments = ""
        for index, comment in enumerate(doc.metadata["list_comments"]):
            comments += f"Comment #{index+1}: {comment}\n"
